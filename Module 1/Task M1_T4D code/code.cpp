...
void pciSetup(byte pin) {
  // it is used to enable or disable individual pins for Pin Change Interrupts.
...
void startTimer(double timerFrequency) {
  ...
  TCCR1A = 0; // It keeps track of how the timer should behave.
  TCCR1B = 0;
  TCNT1 = 0; // It is used in initializing the counter value to zero.
  OCR1A = ocrValue; // used for generating an interrupt or changing the state of an output pin.
  TCCR1B |= (1 << WGM12);
  TCCR1B |= (1 << CS12) | (1 << CS10);
  TIMSK1 |= (1 << OCIE1A); // It is used to enable the interrupt for Output Compare A 
